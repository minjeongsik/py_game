...
public sealed class Game1 : Game
{
    ...
    protected override void Initialize()
    {
        ...
        _player = new PlayerController(new Vector2(spawnTile.X * _worldMap.TileSize, spawnTile.Y * _worldMap.TileSize), 140f);
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        _spriteBatch.Begin();
        UiRenderer.DrawOverlay(
            _spriteBatch,
            _pixel!,
            GraphicsDevice.Viewport,
            _stateManager.CurrentState,
            _player.WorldPosition,
            _worldMap.CurrentZoneName,
            BuildDebugOverlayData());
        _spriteBatch.End();

        base.Draw(gameTime);
    }

    private DebugOverlayData BuildDebugOverlayData()
    {
        var tile = _worldMap.ToTilePosition(_player.WorldPosition);
        return new DebugOverlayData
        {
            State = _stateManager.CurrentState,
            PlayerPosition = _player.WorldPosition,
            LastInputDirection = _player.LastInputDirection,
            InputDetected = _player.InputDetectedThisFrame,
            MovedThisFrame = _player.MovedThisFrame,
            IsWindowActive = IsActive,
            PlayerTile = tile,
            PlayerTileId = _worldMap.GetTileAt(tile.X, tile.Y)
        };
    }

    private void UpdateWindowTitle()
    {
        Window.Title = $"Aether Trail | {_stateManager.CurrentState} | Pos: {_player.WorldPosition.X:0.0},{_player.WorldPosition.Y:0.0} | Input: {(_player.InputDetectedThisFrame ? "YES" : "NO")}";
    }
}
