...
{
    private static readonly Color BorderColor = new(196, 220, 232);
    ...
    public void Update(GameTime gameTime, GameContext context)
    {
        ...
        if (context.Input.WasRepeated(Keys.Left) || context.Input.WasRepeated(Keys.A)) _partyPaneSelected = true;
        if (context.Input.WasRepeated(Keys.Right) || context.Input.WasRepeated(Keys.D) || context.Input.WasPressed(Keys.Tab)) _partyPaneSelected = false;

        if (_partyPaneSelected && party.Count > 0)
        {
            _selectedParty = Math.Clamp(_selectedParty, 0, party.Count - 1);
            if (context.Input.WasRepeated(Keys.Up) || context.Input.WasRepeated(Keys.W)) _selectedParty = (_selectedParty + party.Count - 1) % party.Count;
            if (context.Input.WasRepeated(Keys.Down) || context.Input.WasRepeated(Keys.S)) _selectedParty = (_selectedParty + 1) % party.Count;
            if (context.Input.WasPressed(Keys.Enter) || context.Input.WasPressed(Keys.Space)) { DepositSelected(context); return; }
        }
        else if (!_partyPaneSelected && storage.Count > 0)
        {
            _selectedStorage = Math.Clamp(_selectedStorage, 0, storage.Count - 1);
            if (context.Input.WasRepeated(Keys.Up) || context.Input.WasRepeated(Keys.W)) _selectedStorage = (_selectedStorage + storage.Count - 1) % storage.Count;
            if (context.Input.WasRepeated(Keys.Down) || context.Input.WasRepeated(Keys.S)) _selectedStorage = (_selectedStorage + 1) % storage.Count;
            if (context.Input.WasPressed(Keys.Enter) || context.Input.WasPressed(Keys.Space)) { WithdrawSelected(context); return; }
        }

        if (context.Input.WasPressed(Keys.Escape)) context.StateManager.ChangeState(context.Session.ReturnState);
    }
    ...
    public void Draw(GameTime gameTime, GameContext context)
    {
        ...
        layout.DrawHeader("PC 보관함", $"파티 {party.Count}  박스 {storage.Count}", BorderColor);
        DrawPartyPane(context, party);
        DrawStoragePane(context, storage);
        DrawDetailPane(context, party, storage);
        layout.DrawFooter(_panelMessage, "좌우 화면 선택  위아래 선택  Enter 이동  ESC 돌아가기", BorderColor);
    ...
    {
        var party = context.Session.Party;
        if (party.Count <= 1) { _panelMessage = "파티에는 최소 한 마리가 남아 있어야 합니다."; return; }
    ...
    {
        var party = context.Session.Party;
        if (party.IsFull) { _panelMessage = "파티가 가득 찼습니다. 먼저 맡겨 주세요."; return; }
    ...
    private void DrawPartyPane(GameContext context, Domain.Party.Party party)
    {
        ...
        context.PrimitiveRenderer.Outline(rect, 3, _partyPaneSelected ? new Color(250, 226, 132) : BorderColor);
        context.TextRenderer.DrawText(new Vector2(48, 132), "파티", 3, new Color(238, 246, 252));
        if (party.Count == 0) { context.TextRenderer.DrawText(new Vector2(72, 240), "파티 멤버가 없습니다.", 2, new Color(220, 228, 232)); return; }
        ...
        for (var i = 0; i < party.Members.Count; i++)
        {
            ...
            layout.DrawSelectableRow(new Rectangle(44, y - 8, 320, 52), selected, new Color(72, 98, 116), new Color(28, 38, 52), i == party.ActiveIndex ? new Color(250, 226, 132) : new Color(84, 96, 110), new Color(84, 96, 110));
            ...
            context.TextRenderer.DrawText(new Vector2(108, y + 24), i == party.ActiveIndex ? "선두" : creature.IsFainted ? "기절" : "준비", 2, creature.IsFainted ? new Color(236, 146, 132) : new Color(214, 224, 228));
            y += 62;
        }
    }
    ...
    private void DrawStoragePane(GameContext context, Domain.Party.CreatureStorage storage)
    {
        ...
        context.PrimitiveRenderer.Fill(rect, new Color(18, 26, 36, 236));
        context.PrimitiveRenderer.Outline(rect, 3, !_partyPaneSelected ? new Color(206, 236, 244) : BorderColor);
        context.TextRenderer.DrawText(new Vector2(424, 132), "1번 박스", 3, new Color(238, 246, 252));
        if (storage.Count == 0) { context.TextRenderer.DrawText(new Vector2(520, 240), "보관함이 비어 있습니다.", 2, new Color(220, 228, 232)); return; }
        ...
        for (var i = 0; i < storage.Count; i++)
        {
            ...
            layout.DrawSelectableRow(new Rectangle(420, y - 8, 240, 52), selected, new Color(72, 98, 116), new Color(28, 38, 52), new Color(206, 236, 244), new Color(84, 96, 110));
    ...
    {
        var rect = new Rectangle(678, 174, 232, 282);
        context.PrimitiveRenderer.Fill(rect, new Color(24, 34, 44, 244));
        context.PrimitiveRenderer.Outline(rect, 3, BorderColor);
        context.TextRenderer.DrawText(new Vector2(700, 192), "상세", 3, new Color(238, 246, 252));

        Creature? current = null;
        var actionHint = "몬스터를 선택하세요";
        if (_partyPaneSelected && party.Count > 0)
        {
            current = party.Members[_selectedParty];
            actionHint = party.Count <= 1 ? "마지막 파티 멤버는 맡길 수 없습니다." : "Enter로 보관함에 맡기기";
        }
        else if (!_partyPaneSelected && storage.Count > 0)
        {
            current = storage.Creatures[_selectedStorage];
            actionHint = party.IsFull ? "파티가 가득 찼습니다." : "Enter로 파티에 꺼내기";
        }
        ...
        context.TextRenderer.DrawText(new Vector2(706, 456), "EXP", 2, new Color(214, 224, 228));
        context.UiSkin.DrawExperienceBar(context.SpriteBatch, new Rectangle(756, 460, 124, 12), GetExpRatio(current));
        context.TextRenderer.DrawText(new Vector2(706, 482), $"남은 EXP {GetExpRemaining(current)}", 1, new Color(214, 224, 228));
        context.TextRenderer.DrawText(new Vector2(706, 500), actionHint, 1, new Color(236, 236, 224));
    }
