...
public sealed class ShopState : IGameState
{
    ...
    private static readonly Color BorderColor = new(214, 188, 108);
    private int _selected;
    private string _message = "길에 필요한 물건을 사 가세요.";
    ...
    public void Update(GameTime gameTime, GameContext context)
    {
        ...
        if (context.Input.WasPressed(Keys.Up) || context.Input.WasPressed(Keys.W)) _selected = (_selected + shopItemIds.Count - 1) % shopItemIds.Count;
        if (context.Input.WasPressed(Keys.Down) || context.Input.WasPressed(Keys.S)) _selected = (_selected + 1) % shopItemIds.Count;
    ...
    public void Draw(GameTime gameTime, GameContext context)
    {
        ...
        layout.DrawHeader("상점", $"소지금 {context.Session.Money}", BorderColor);
        ...
        for (var i = 0; i < shopItemIds.Count; i++)
        {
            ...
            layout.DrawSelectableRow(new Rectangle(44, y - 8, 480, 52), i == _selected, new Color(88, 84, 62), new Color(44, 38, 28), new Color(250, 226, 132), new Color(118, 108, 82));
        ...
        if (shopItemIds.Count > 0)
        {
            ...
            context.TextRenderer.DrawText(new Vector2(604, 436), "Enter로 1개 구입", 2, new Color(236, 236, 224));
        }

        layout.DrawFooter(_message, "위아래 선택  Enter 구입  ESC 돌아가기", BorderColor);
